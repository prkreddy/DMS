...
<body>
	...
	<s:form id="frmDocEdit" action="documentUploadAction" method="POST"
		...
		<div style="float:left;">
			...
			<s:submit id='b2' value='>>' onclick='moveAllOptions1(); return false;' />
			<s:submit id='b3' value='<' onclick='moveSelectedOptions1Back(); return false;' />
	...
	<script type="text/javascript">
		...
		function moveSelectedOptionUp()
		{
			...
			for (var i=0; i<l1.options.length; i++)
				if(i>0 && l1.options[i].selected==true && l1.options[i-1].selected==false)
				{
		...
		function moveSelectedOptionDn()
		{
			...
			for (var i=l1.options.length-1; i>=0; i--)
				if(i<l1.options.length-1 && l1.options[i].selected==true && l1.options[i+1].selected==false)
				{
			...
			for (var i=l1.options.length-1; i>=0; i--)
				if(l1.options[i].selected)
				{
					...
					for(var k=0; k<l2.options.length; k++)
						if(l2.options[k].text.split(",")[0]==l1.options[i].text.split(",")[0])
						{
							moveFlag=false;
